...
async function coeurVote(data) {
  ...
  let coeurFirst = document.querySelector(`.coeurFirst${data.id}`);
  let coeurSecond = document.querySelector(`.coeurSecond${data.id}`);
  ...
  coeurId.addEventListener("click", () => {
    if (vote == false) {
      ...
      coeurSecond.style.opacity = "1";
      return voteLike;
    } else {
      vote = false;
      voteLike = data.likes;
      coeurSecond.style.opacity = "0";
      return voteLike;
    }
  });
}
